...
export URL_POST=https://go-global-02.architecture.caradhras.io/ledger/movimentStatementPerDate
export URL_POST2=https://go-global-02.architecture.caradhras.io/ledger/movimentStatement
...
# POST request
curl -s -i -X GET ''$URL_POST'?account-id=ACC-'$RANDOM_ACC'&date_start=2025-04-07' \
	--header "Content-Type: application/json" \
	--header "Authorization: $AUTH_TOKEN" | grep "^HTTP\/"\

curl -s -i -X GET ''$URL_POST2'/ACC-'$RANDOM_ACC'' \
	--header "Content-Type: application/json" \
	--header "Authorization: $AUTH_TOKEN" | grep "^HTTP\/"\
